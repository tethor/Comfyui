...
# ==================================================================================
# MEJORAS DE GESTIÓN DE MEMORIA RAM - VERSIÓN COMPLETA Y COMPATIBLE
# ==================================================================================
# Agregado por: tethor
# Fecha: 2025-11-06
# Propósito: Liberar memoria RAM después de cada generación de imagen/video
# ==================================================================================
...
def aggressive_memory_cleanup(log_stats=True):
    """
    Limpieza agresiva de memoria RAM y VRAM después de cada generación.
    """
    try:
        ...
        # DisTorch2/MultiGPU metadata cleanup
        try:
            # Check if MultiGPU/DisTorch2 is loaded
            if any('multigpu' in str(mod).lower() or 'distorch' in str(mod).lower()
                ...
                for lm in comfy.model_management.current_loaded_models:
                    try:
                        ...
                        pass
                if cleaned_count > 0:
                    logging.info(f"[MEMORY CLEANUP] Cleaned DisTorch2 metadata from {cleaned_count} models")
        except Exception as e:
            logging.debug(f"[MEMORY CLEANUP] DisTorch2 check: {e}")

        # Limpiar VRAM (multi-device support)
...
# PARA APLICAR LOS PARCHES, DEBEMOS REEMPLAZAR LA FUNCIÓN prompt_worker
# GUARDAMOS LA ORIGINAL Y CREAMOS UNA NUEVA QUE INCLUYA LA LIMPIEZA

_original_prompt_worker = prompt_worker

def prompt_worker(q, server_instance):
    """
    Versión mejorada del worker con limpieza automática después de cada generación.
    """
    current_time: float = 0.0
    cache_type = execution.CacheType.CLASSIC
    if args.cache_lru > 0:
    ...
    while True:
        ...
        if queue_item is not None:
            ...
            # ===== LIMPIEZA AGRESIVA DESPUÉS DE CADA GENERACIÓN =====
...
# ADEMÁS, PARCHEAMOS LA FUNCIÓN start_comfyui PARA AÑADIR ENDPOINTS API
_original_start_comfyui = start_comfyui

def start_comfyui(asyncio_loop=None):
    """
    Versión mejorada que añade endpoints API para control de memoria.
    """
    if args.temp_directory:
        temp_dir = os.path.join(os.path.abspath(args.temp_directory), "temp")
        logging.info(f"Setting temp directory to: {temp_dir}")
    ...
    # ===== AÑADIR ENDPOINTS API PARA MEMORIA =====
    try:
        @prompt_server.routes.post("/api/memory/cleanup")
        async def memory_cleanup(request):
            """Endpoint para liberar memoria manualmente."""
            try:
                ram_freed = aggressive_memory_cleanup()
                return prompt_server.jsonify({
                    "status": "success",
                    "ram_freed_gb": round(ram_freed, 2),
                    "message": f"Liberados {ram_freed:.2f} GB de RAM"
                })
            except Exception as e:
                return prompt_server.jsonify({
                    "status": "error",
...
logging.info("[MEMORY OPTIMIZATION] Sistema de limpieza agresiva de memoria inicializado")
logging.info("[MEMORY OPTIMIZATION] La memoria se liberará automáticamente después de cada generación")

# Fin de las mejoras de gestión de memoria
